...
We will see a quick setup to configure the ``rsyslog`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``rsyslog`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``rsyslog`` formulas under ``/srv/salt/rsyslog/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/rsyslog/parameters/dns:domain/
    ln -s dns:domain /srv/salt/rsyslog/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/rsyslog/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/rsyslog/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``rsyslog`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``rsyslog`` formulas under ``/srv/salt/rsyslog/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/rsyslog/parameters/roles
...
We create a configuration for the role ``rsyslog/server`` in ``/srv/salt/rsyslog/parameters/roles/rsyslog/server.yaml``:
...
We create another configuration for the role ``rsyslog/client`` in ``/srv/salt/rsyslog/parameters/roles/rsyslog/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles rsyslog/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles rsyslog/client
...
.. code-block:: console
    ...
    mv /srv/salt/rsyslog/parameters/dns:domain/  '/srv/salt/rsyslog/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-rsyslog-as-json:
      test.nop:
        - name: {{ rsyslog | json }}
...
Here is an example based on `template-formula/rsyslog/config/file.sls`_:
...
This ``sls`` file expose a ``rsyslog`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ rsyslog['winner'] }}
...
.. _template-formula/rsyslog/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/rsyslog/config/file.sls
